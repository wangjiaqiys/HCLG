...
def save_word_dict(vocab, save_path):
    with open(save_path, 'w', encoding='utf-8') as f:
        for line in vocab:
            ...
            f.write("%s\t%d\n" % (w, i))
"""
def save_word_dict(vocab, save_path, sflag):
    # TODO: 增加sflag，sflag=0 - save vocab; sflag=1 - save reverse_vocab
    if sflag == 0:
        with open(save_path, 'w', encoding='utf-8') as f:
            for line in vocab:
                w, i = line
...
def build_vocab(items, sort=True, min_count=0, lower=False):
    ...
    if sort:
        ...
        for i, item in enumerate(items):
            item = item if not lower else item.lower() # 'T'.lower(), '我'.lower()->'我'
            result.append(item)
    """
    ...
    """
    # TODO:
    vocab = [(word, value) for value, word in enumerate(result)]
    reverse_vocab = [(value, word) for value, word in enumerate(result)]
...
if __name__ == '__main__':
    ...
    save_word_dict(reverse_vocab, '{}/data/reverse_vocab.txt'.format(BASE_DIR), 1)