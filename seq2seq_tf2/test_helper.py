...
def greedy_decode(model, dataset, vocab, params):
    ...
    sample_size = 20000 # 20000条样本
    # batch 操作轮数 math.ceil向上取整 小数 +1
    # 因为最后一个batch可能不足一个batch size 大小 ,但是依然需要计算
    steps_epoch = sample_size // batch_size + 1
    
    for i in tqdm(range(steps_epoch)):
        enc_data, _ = next(iter(dataset)) # enc_data.shape (64, 200) - (batch_size, sequence_len)
...
def batch_greedy_decode(model, enc_data, vocab, params):
    """
    enc_data: dict, keys(enc_len, max_oov_len, enc_input, extended_enc_inpu, article_oovs, article, sample_encoder_pad_mask)
    """
    # 判断输入长度
    batch_data = enc_data["enc_input"]
    batch_size = enc_data["enc_input"].shape[0] # 参数里设置的batch_size是64
    # 开辟结果存储list
    predicts = [''] * batch_size
    ...
    # enc_output.shape: [batch_size, sequence_len, embedding_dim] [64, 200, 256]
    # enc_hidden: [batch_size, 2*enc_unit] # 前一个状态和后一个状态的拼接
    # import pdb; pdb.set_trace()
    ...
    dec_input = tf.constant([2] * batch_size) # [2] start标记, shape (64, ) 向量
    # dec_input.shape [64, 1]
    dec_input = tf.expand_dims(dec_input, axis=1) # 增加维度,变成一个矩阵，相当于batch_size个句子，每个句子开头为2
    # 计算 context_vector, shape[batch_size, 2*enc_unit] = [64, 256]
    context_vector, _ = model.attention(dec_hidden, enc_output) # 这句话的作用？
    # import pdb; pdb.set_trace()
    for t in range(params['max_dec_len']): # 解码最大长度为： 40
        # 单步预测
        """
        ...
        # decoder
        # dec_input.shape [64, 1]
        # dec_hidden.shape [64, 256]
        # enc_output.shape [64, 200, 256]
        # context_vector.shape [64, 256]
        _, predict_result, _ = model.decoder(dec_input,dec_hidden,enc_output,context_vector)
        # predicts.shape [64, 1,]
        ...
        """
        predicted_ids = tf.argmax(predict_result,axis=1).numpy()
        # import pdb; pdb.set_trace()
